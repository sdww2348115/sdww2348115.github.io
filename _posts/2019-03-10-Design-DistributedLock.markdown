...
线程获取锁资源时，如果线程获取锁资源失败，就在等待队列的末尾添加一个节点，代表自己正在排队等待锁资源释放；接着按照预设的逻辑进行等待。当线程释放锁资源时，如果等待队列中有线程存在，则唤醒下一个等待的线程。
等待队列的第一个问题是：对于释放锁资源的唤醒信号，应由释放锁资源的线程'推送'给下一个等待线程还是由等待线程自己去获取？假设采用推送的模式，此时线程A与线程B都需要独占锁资源才能向下继续执行，它们各自执行情况如下：
